...
from codex_wire_audit.system_contract import build_system_contract, digest
...
def combined_model():
    ...
    registry = SourceRegistry([replace(spec, path_candidates=(files[spec.id].selected_path,)) if spec.id in files else spec for spec in registry.specs])
...
@pytest.mark.parametrize('mutation,match', [(lambda contract: contract.update(schema='codex-system-contract/v2'), 'schema violation'), (lambda contract: contract.update(extra='unexpected'), 'schema violation'), (lambda contract: contract['integrity'].update(canonical_sha256='0' * 64), 'content digest mismatch')])
...
@pytest.mark.parametrize('mutation,match', [(lambda m: m['migration']['canonical_ir_extractors'].clear(), 'coverage mismatch'), (lambda m: m['extractors']['extractor.turn_metadata'].update(extractor_id='extractor.other'), 'schema violation|key/id mismatch'), (lambda m: m['extractors']['extractor.turn_metadata']['source_spec_ids'].append('source_spec.unknown'), 'undeclared source'), (lambda m: m['source_snapshot']['counts'].update(available=99), 'count mismatch'), (lambda m: m['source_snapshot']['revision'].update(requested_ref='different'), 'revision disagreement')])
...
def test_new_extractor_is_not_dropped_by_a_hard_coded_surface_catalog():
    model = fixture_contract()['model']
    model['extractors']['extractor.future'] = {'extractor_id': 'extractor.future', 'schema_version': '1.0.0', 'source_spec_ids': ['source_spec.base.metadata'], 'semantic_complete': True, 'data': {'new_fact': 'preserved'}}
