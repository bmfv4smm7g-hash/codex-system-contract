...
SOURCE_TEXTS.update({
    ...
    "live_writer": "// SQLite is a rebuildable view.\nasync fn durable_write() {}\nmaterialize_to_sqlite();\n",
    "rollout_resolver": "// SQLite's selected rollout path is authoritative.\nfn resolve_current() {}\nenum LookupScope { ExcludeArchived, IncludeArchived }\n",
    "model_context": "fn load_latest_model_context() {}\nstruct ReverseJsonlScanner;\nresolve_rollout_lineage();\n",
...
def test_release_spec_declares_local_storage_capability_closure() -> None:
    ...
    assert 'proof_schema_templates/local-storage-semantics-v2.schema.json' in local['required_resources']


def test_local_storage_restart_projection_semantics_are_explicit() -> None:
    result, _ = _extract()
    assert result.data["read_paths"]["paginated_query_history"]["full_replay_on_restart"] is False
    p = result.data["projection_semantics"]
    assert p["role"] == "persistent rebuildable acceleration view"
    assert p["write_order"] == ["durable JSONL write", "incremental SQLite projection"]
    assert p["checkpoint"] == ["next_rollout_byte_offset", "next_rollout_ordinal"]
    assert p["external_rewrite"] == {"supported": False, "shrink_error": "durable rollout shrank before projection", "automatic_reset": False}
    r = result.data["transitions"]["revert"]
