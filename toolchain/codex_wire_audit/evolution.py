...
def _gate_descriptions(turn_data: Mapping[str, Any]) -> dict[str, str]:
    ...
    for name, gate in (turn_data.get('gates') or {}).items():
        ...
        if name == 'has_thread_identity':
            descriptions[name] = f'thread-scoped identity is emitted when request_kind is absent or the request kind accepts thread identity; source expression: {expression}'
        elif name == 'has_turn_identity':
            descriptions[name] = 'legacy combined identity gate; source expression: ' + str(expression)
        elif name == 'has_request_identity':
            descriptions[name] = f'request-scoped identity is emitted only when request_kind is present and its kind accepts thread identity; source expression: {expression}'
        else:
            descriptions[name] = f'source-derived gate; expression: {expression}'
    return descriptions

def apply_turn_metadata_overlay(report: MutableMapping[str, Any], turn_result: ExtractorResult) -> None:
    ...
    construction['fields'] = [{'name': field_name, 'construction_rule': _description_for_emission(field), 'expression': field.get('source_expression'), 'emission': copy.deepcopy(field), 'source': {'path': data.get('source_path'), 'symbol': field.get('source_ref')}} for field_name, field in (data.get('fields') or {}).items()]
    ...
    if isinstance(schema, MutableMapping):
        notes = [note for note in schema.get('notes') or [] if 'has_turn_identity' not in str(note)]
        notes.append('Identity emission is source-derived per field: request identity, thread identity, turn identity, and lineage are distinct domains.')
        schema['notes'] = notes
        schema['identity_model'] = {'field_groups': copy.deepcopy(data.get('field_groups') or {}), 'gate_refs': sorted((data.get('gates') or {}).keys()), 'semantic_digest': data.get('semantic_digest')}
    ...
    if isinstance(metadata_protocol, MutableMapping):
        ...
        if isinstance(nested, MutableMapping):
            ...
            if isinstance(groups, MutableMapping):
                groups['request_identity'] = list((data.get('field_groups') or {}).get('request_identity', []))
                groups['thread_identity'] = list((data.get('field_groups') or {}).get('thread_identity', []))
                groups['turn_identity'] = list((data.get('field_groups') or {}).get('turn_identity', []))
                groups['legacy_combined_turn_identity'] = {'deprecated': True, 'fields': sorted(set(groups.get('thread_identity', [])) | set(groups.get('turn_identity', []))), 'replacement': ['thread_identity', 'turn_identity']}
    ...
    if isinstance(mcp_protocol, MutableMapping):
        ...
        if isinstance(projection, MutableMapping):
            ...
            if isinstance(identity, MutableMapping):
                ...
                identity['thread_identity_fields'] = list((data.get('field_groups') or {}).get('thread_identity', []))
                identity['turn_identity_fields'] = list((data.get('field_groups') or {}).get('turn_identity', []))
                identity['legacy_has_turn_identity'] = {'deprecated': True, 'replacement': ['has_thread_identity', 'turn_identity_independent']}
    ...
    if isinstance(relation_map, MutableMapping):
        ...
        if isinstance(relation_types, MutableMapping):
            relation_types['same_value_when_thread_identity'] = 'same value when thread identity is emitted'
            relation_types['same_value_when_turn_identity'] = 'same value when the independent turn identity value exists'
    ...
    report['turn_metadata_semantics'] = {'authoritative': True, 'extractor_id': turn_result.extractor_id, 'schema_version': turn_result.schema_version, **copy.deepcopy(data)}
...
def build_evolution_contract(snapshot: SourceSnapshot, registry: SourceRegistry, diagnostics: DiagnosticCollector, *, coverage_profile: str='codex_wire_full', legacy_report: Mapping[str, Any] | None=None) -> tuple[dict[str, Any], dict[str, ExtractorResult]]:
    ...
    if config_result is not None and config_result.data:
        ...
        if not graph_complete:
            diagnostics.emit(code='CONFIG_SURFACE_GRAPH_REFERENCE_UNRESOLVED', severity='error', category='config_surface', message='The config-to-surface graph contains unresolved node references.', extractor_id='extractor.config_effects', entity_id='config_surface_graph', details={'node_refs': (graph.get('coverage') or {}).get('unresolved_node_refs')}, recoverable=False, strict_failure=True)
        config_data['semantic_complete'] = config_result.semantic_complete and graph_complete
        config_data['semantic_digest'] = hashlib.sha256(canonical_json_bytes({key: value for key, value in config_data.items() if key != 'semantic_digest'})).hexdigest()
        results[config_result.extractor_id] = ExtractorResult(extractor_id=config_result.extractor_id, schema_version=config_result.schema_version, data=config_data, semantic_complete=config_result.semantic_complete and graph_complete, source_spec_ids=config_result.source_spec_ids)
...
def finalize_evolution_contract(contract: MutableMapping[str, Any], diagnostics: DiagnosticCollector, *, schema_resolution_complete: bool) -> None:
