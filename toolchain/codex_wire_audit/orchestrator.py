...
from .evolution import apply_turn_metadata_overlay, build_evolution_contract, finalize_evolution_contract, finalize_system_report
...
from .sources import SnapshotLoadResult, SourceLoadError, load_archive_snapshot, load_cache_snapshot, load_github_snapshot, load_repo_snapshot
...
def load_snapshot(args: Any, legacy: LegacyModules, registry: SourceRegistry) -> SnapshotLoadResult:
    if getattr(args, 'repo_root', None):
        return load_repo_snapshot(registry, args.repo_root, repository=args.repo, requested_ref=args.ref, source_commit=getattr(args, 'source_commit', None), allow_dirty=getattr(args, 'allow_dirty_source', False), worktree=getattr(args, 'worktree', False))
    if getattr(args, 'source_archive', None):
        return load_archive_snapshot(registry, args.source_archive, repository=args.repo, requested_ref=args.ref, source_commit=getattr(args, 'source_commit', None))
    if getattr(args, 'cache_only', False):
        return load_cache_snapshot(registry, cache_dir=args.cache_dir, repository=args.repo, commit_sha=args.ref)
    return load_github_snapshot(registry, repository=args.repo, requested_ref=args.ref, api_base=args.api_base, token=args.token, legacy_base=legacy.base, max_workers=getattr(args, 'fetch_workers', 8))

def _legacy_warning_lists(snapshot: SourceSnapshot, registry: SourceRegistry) -> tuple[list[str], list[str]]:
    surface: list[str] = []
    extra: list[str] = []
    for spec_id, reason in sorted(snapshot.unavailable_specs.items()):
        spec = registry.get(spec_id)
        if spec.group == SourceGroup.SURFACE:
            surface.append(f'optional wire surface source unavailable: {spec.primary_path}: {reason}')
...
def combined_source_descriptors(snapshot: SourceSnapshot, registry: SourceRegistry) -> dict[str, dict[str, Any]]:
    ...
    for spec in registry.specs:
        ...
        if source_file is None:
            combined[primary] = {'text': None, 'roles': roles, 'available': False, 'source_spec_id': spec.id, 'path_candidates': list(spec.path_candidates)}
            continue
        combined[primary] = {'text': source_file.text, 'roles': roles, 'available': True, 'source_spec_id': spec.id, 'resolved_path': source_file.selected_path, 'logical_path_alias': primary != source_file.selected_path}
        if primary != source_file.selected_path:
            combined[source_file.selected_path] = {'text': source_file.text, 'roles': [*roles, 'resolved_path'], 'available': True, 'source_spec_id': spec.id, 'resolved_path': source_file.selected_path, 'logical_path_alias': False}
    return combined

def _patch_exact_source_manifest(report: MutableMapping[str, Any], snapshot: SourceSnapshot, registry: SourceRegistry) -> None:
    ...
    for spec in registry.specs:
        ...
        for path in {spec.primary_path, source_file.selected_path}:
            ...
            if not isinstance(entry, MutableMapping):
                continue
            entry.update({'byte_length': len(source_file.raw_bytes), 'line_count': source_file.text.count('\n') + (0 if not source_file.text or source_file.text.endswith('\n') else 1), 'sha256': source_file.content_sha256, 'git_blob_sha': source_file.git_blob_sha, 'repository_blob_sha': source_file.repository_blob_sha, 'byte_identity': 'exact', 'source_spec_id': spec.id, 'resolved_path': source_file.selected_path, 'logical_path_alias': path != source_file.selected_path, 'path_candidate_index': source_file.path_candidate_index})

def _annotate_qualified_schemas(report: MutableMapping[str, Any], resolver: SchemaIdentityResolver, diagnostics: DiagnosticCollector) -> None:
    ...
    for schema_id, metadata in resolver.metadata.items():
        ...
        if isinstance(schema, MutableMapping):
            schema.update({'rust_name': metadata.get('rust_name'), 'qualified_rust_name': metadata.get('qualified_rust_name'), 'source_path': metadata.get('source_path'), 'schema_identity_fallback': bool(metadata.get('fallback'))})
            shape = {'field_ids': schema.get('field_ids') or [], 'variants': schema.get('variants') or [], 'kind': schema.get('kind')}
            schema['shape_fingerprint'] = hashlib.sha256(canonical_json_bytes(shape)).hexdigest()
        ...
        if isinstance(value, MutableMapping):
            ...
            if isinstance(ref, str) and ref.startswith('schema.rust.'):
                ...
                if len(candidates) == 1:
                    ...
                    diagnostics.emit(code='NAMED_TYPE_REFERENCE_AMBIGUOUS', severity='warning', category='schema_resolution', message=f'A named Rust type reference is ambiguous: {leaf}', extractor_id='schema_identity', entity_id=field_id, details={'name': leaf, 'candidate_schema_ids': sorted(candidates)}, strict_failure=True)
...
def _update_report_status(report: MutableMapping[str, Any], evolution_contract: Mapping[str, Any]) -> None:
    ...
    status['semantic_classification_complete'] = dimensions.get('semantic_classification') == 'complete'
    status['source_revision_id'] = (evolution_contract.get('source_revision') or {}).get('source_revision_id')
    status['diagnostic_refs'] = [item.get('id') for item in report.get('diagnostics', []) if isinstance(item, Mapping) and item.get('id')]

def generate_report(args: Any, legacy: LegacyModules, *, baseline_report: Mapping[str, Any] | None=None) -> GenerationResult:
    ...
    try:
        report = contract_module.upgrade_report(report, commit=loaded.commit, sources=combined_source_descriptors(snapshot, registry), source_mode=snapshot.revision.source_mode, repository=args.repo, requested_ref=snapshot.revision.requested_ref, dirty=snapshot.revision.dirty, parser_backend=getattr(args, 'parser', 'auto'))
    finally:
    ...
    if baseline_report is not None:
        try:
            ...
            diagnostics.emit(code='BASELINE_EVOLUTION_CONTRACT_UNAVAILABLE', severity='error', category='semantic_diff', message='The baseline report cannot be used for semantic comparison.', extractor_id='semantic_diff', details={'error': str(error)}, recoverable=False, strict_failure=True)
    schema_resolution_complete = not any((item.severity == 'error' and item.category == 'schema_resolution' for item in diagnostics.values()))
    finalize_evolution_contract(evolution_contract, diagnostics, schema_resolution_complete=schema_resolution_complete)
    ...
    if isinstance(parser, MutableMapping):
        parser['semantic_extractors'] = {'extractor.turn_metadata': {'backend': 'rust_lexical_scanner_plus_expression_classifier', 'machine_evaluable': turn_result.semantic_complete if turn_result else False}, 'extractor.context_management': {'backend': 'source_linked_context_management_classifier', 'machine_evaluable': bool(extractor_results.get('extractor.context_management') and extractor_results['extractor.context_management'].semantic_complete)}, 'extractor.config_effects': {'backend': 'generated_json_schema_plus_feature_registry_surface_graph', 'machine_evaluable': config_result.semantic_complete if config_result else False}, 'extractor.local_storage': {'backend': 'source_linked_local_storage_layout_classifier', 'machine_evaluable': local_storage_result.semantic_complete if local_storage_result else False}}
